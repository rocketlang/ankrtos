...
export interface TradeArea {
  name: string;
  description: string;
  boundingBox: [[number, number], [number, number]]; // [[lat1, lon1], [lat2, lon2]]
  priority: number;
}

export const MAJOR_TRADE_AREAS: TradeArea[] = [
  // ASIA-PACIFIC
  {
...
export const AIS_PRESETS = {
  ...
  major_hubs: {
    ...
    boundingBoxes: MAJOR_TRADE_AREAS.filter(area => area.priority === 1).map(
      area => area.boundingBox
    ),
  },
  ...
  high_priority: {
    ...
    boundingBoxes: MAJOR_TRADE_AREAS.filter(area => area.priority <= 2).map(
      area => area.boundingBox
    ),
  },
  ...
  asia_pacific: {
    ...
    boundingBoxes: MAJOR_TRADE_AREAS.filter(
      area =>
        area.name.includes('China') ||
        area.name.includes('Singapore') ||
        area.name.includes('Taiwan') ||
        area.name.includes('Bay of Bengal') ||
        area.name.includes('Arabian Sea')
  ...
  middle_east: {
    ...
    boundingBoxes: MAJOR_TRADE_AREAS.filter(
      area =>
        area.name.includes('Persian') ||
        area.name.includes('Red Sea') ||
        area.name.includes('Suez') ||
        area.name.includes('Arabian')
    ).map(area => area.boundingBox),
  },
  ...
  user_specified: {
    ...
    boundingBoxes: MAJOR_TRADE_AREAS.filter(area =>
      [
        ...
        'US West Coast',
        'US East Coast',
      ].some(name => area.name.includes(name.split(' ')[0]))
    ).map(area => area.boundingBox),
  },
};

// Helper function to print area configuration
export function printAreaConfig(preset: keyof typeof AIS_PRESETS) {
...
function estimateCoverage(boxes: number[][][]): string {
  // Rough estimate of coverage area
  let totalArea = 0;
  for (const box of boxes) {
    const [[lat1, lon1], [lat2, lon2]] = box as [[number, number], [number, number]];
