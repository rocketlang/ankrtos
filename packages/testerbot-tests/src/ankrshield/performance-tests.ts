...
export const ankrshieldPerformanceTests: Test[] = [
  {
    ...
    fn: async (agent: DesktopTestAgent) => {
      ...
      if (metrics.startupTime > THRESHOLDS.STARTUP_TIME_MS) {
        throw new Error(
          `Startup time ${metrics.startupTime}ms exceeds threshold of ${THRESHOLDS.STARTUP_TIME_MS}ms`
        );
      }
...

  {
    ...
    fn: async (agent: DesktopTestAgent) => {
      ...
      if (memoryBytes > THRESHOLDS.MEMORY_USAGE_BYTES) {
        throw new Error(
          `Memory usage ${memoryMB.toFixed(2)}MB exceeds threshold of ${THRESHOLDS.MEMORY_USAGE_MB}MB`
        );
      }
...

  {
    ...
    fn: async (agent: DesktopTestAgent) => {
      ...
      if (afterBytes > THRESHOLDS.MEMORY_USAGE_BYTES) {
        throw new Error(
          `Memory usage ${afterMB.toFixed(2)}MB exceeds threshold of ${THRESHOLDS.MEMORY_USAGE_MB}MB`
        );
      }
    }
  },

  {
    ...
    fn: async (agent: DesktopTestAgent) => {
      ...
      if (bundleSizeKB > electronThresholdKB) {
        console.warn(
          `⚠️  Bundle size ${bundleSizeKB.toFixed(2)}KB exceeds Electron threshold of ${electronThresholdKB}KB`
        );
...

  {
    ...
    fn: async (agent: DesktopTestAgent) => {
      ...
      if (renderTime > THRESHOLDS.DASHBOARD_RENDER_MS) {
        throw new Error(
          `Dashboard render time ${renderTime}ms exceeds threshold of ${THRESHOLDS.DASHBOARD_RENDER_MS}ms`
        );
      }
    ...
    fn: async (agent: DesktopTestAgent) => {
      ...
      for (const interaction of interactions) {
        ...
        try {
          ...
          console.warn(`⚠️  ${interaction.name} failed:`, (err as Error).message);
          continue;
        }
      ...
      if (results.length > 0) {
        ...
        if (avgTime > THRESHOLDS.INTERACTION_RESPONSE_MS) {
          throw new Error(
            `Average interaction response ${avgTime.toFixed(0)}ms exceeds threshold of ${THRESHOLDS.INTERACTION_RESPONSE_MS}ms`
          );
        }
...

  {
    ...
    fn: async (agent: DesktopTestAgent) => {
      ...
      if (totalTime > THRESHOLDS.DASHBOARD_RENDER_MS) {
        throw new Error(
          `Total progressive load time ${totalTime}ms exceeds threshold of ${THRESHOLDS.DASHBOARD_RENDER_MS}ms`
        );
      }
