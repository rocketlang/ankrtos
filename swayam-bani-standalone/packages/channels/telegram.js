...
async function handleMessage(msg) {
    ...
    try {
        ...
        await (0, router_1.logChannelInteraction)('telegram', userId, 'incoming', true);
        // Send typing indicator
        await sendChatAction(chatId, 'typing');
        const response = await (0, router_1.processMessage)(message);
        await sendFormattedResponse(chatId, response);
        await (0, router_1.logChannelInteraction)('telegram', userId, 'outgoing', true);
    }
    catch (error) {
        console.error('[Telegram] Message handling error:', error);
        await sendMessage(chatId, '❌ Something went wrong. Please try again.');
        await (0, router_1.logChannelInteraction)('telegram', userId, 'outgoing', false);
    }
}
/**
 ...
 */
async function sendFormattedResponse(chatId, response) {
 ...
 * Send voice message
 */
async function sendVoice(chatId, audioBase64) {
    try {
...
async function transcribeVoice(fileId) {
    try {
        // Get file path
        const fileResponse = await fetch(`${TELEGRAM_API}${config.botToken}/getFile?file_id=${fileId}`);
 ...
 */
async function sendProactiveMessage(chatId, text, buttons) {
