...
export async function getOrCreateSession(
  ...
  try {
    ...
    const result = await pool.query(
      `SELECT * FROM channel_sessions WHERE session_id = $1`,
      [sessionId]
    );
    ...
    if (result.rows.length > 0) {
      // Update last active
      await pool.query(
        `UPDATE channel_sessions SET last_active_at = NOW() WHERE session_id = $1`,
        [sessionId]
      );
      return {
    ...
    await pool.query(
      ...
      [sessionId, channel, channelUserId, channelChatId, persona, language, []]
    );
...
export async function updateSession(
  sessionId: string,
  updates: { persona?: string; language?: string; context?: string[] }
): Promise<void> {
  ...
  await pool.query(
    `UPDATE channel_sessions SET ${setClauses.join(', ')} WHERE session_id = $${paramIndex}`,
    values
  );
}
...
export async function processMessage(
  ...
  const session = await getOrCreateSession(
    ...
    message.channelChatId,
    { persona: message.persona, language: message.language }
  );
 ...
 */
function getLanguageChangedMessage(language: string): string {
  const messages: Record<string, string> = {
 ...
 */
export async function logChannelInteraction(
  channel: Channel,
  channelUserId: string,
  ...
  try {
    await pool.query(
      `INSERT INTO channel_analytics (channel, channel_user_id, message_type, success)
       VALUES ($1, $2, $3, $4)`,
      [channel, channelUserId, messageType, success]
    );
  } catch (error) {
