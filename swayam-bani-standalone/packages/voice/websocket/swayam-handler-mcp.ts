...
import {
  ...
  type TodoPlan,
  type ProgressEvent,
} from '@ankr/intelligence';

// ============================================================================
// INLINE TYPES (to avoid external dependency issues)
// ============================================================================

type SupportedLanguage = 'hi' | 'en' | 'bn' | 'ta' | 'te' | 'mr' | 'gu' | 'kn' | 'ml' | 'pa' | 'od';
...
class SarvamSTT implements STTProvider {
  name = 'sarvam';
  ...
  async transcribe(audio: Buffer, language: SupportedLanguage): Promise<STTResult> {
    try {
      ...
      const data = await response.json() as { transcript?: string; text?: string; confidence?: number };
      return {
...
class SarvamTTS implements TTSProvider {
  ...
  constructor(config: { apiKey?: string; serverUrl?: string; defaultVoice?: string }) {
    this.apiKey = config.apiKey;
    this.serverUrl = config.serverUrl || BANI_URL;
    this.defaultVoice = config.defaultVoice || 'anushka';
  }

  async synthesize(text: string, language: SupportedLanguage, voice?: string): Promise<TTSResult> {
    try {
      ...
      const data = await response.json() as { audio?: string; audioBase64?: string; duration?: number };
...
const STTFactory = {
  create(provider: string, config: { apiKey?: string; serverUrl?: string }): STTProvider {
    return new SarvamSTT(config);
  },
};

const TTSFactory = {
  create(provider: string, config: { apiKey?: string; serverUrl?: string; defaultVoice?: string }): TTSProvider {
...
interface SwayamMCPConfig {
  ...
  mcp?: {
    enabled?: boolean;
    logisticsRagUrl?: string;  // For LogisticsRAG API (default: http://localhost:4005)
    databaseUrl?: string;       // For EON context engine
  };
  wakeWord?: {
    enabled?: boolean;
    keywords?: string[];       // Default: ['hey swayam', 'swayam']
    sensitivity?: number;      // 0.0 - 1.0 (default: 0.7)
  };
  streaming?: {
    enabled?: boolean;
    chunkSize?: number;        // Characters per chunk (default: 150)
    maxConcurrent?: number;    // Max parallel TTS requests (default: 3)
  };
  intelligence?: {
    enabled?: boolean;         // Enable @ankr/intelligence (default: true)
    verdaccioUrl?: string;     // Verdaccio package registry (default: http://localhost:4873)
    autoExecute?: boolean;     // Auto-execute plans without confirmation (default: false)
  };
...
interface PersonaConfig {
  ...
  voice?: string;
  greeting?: string;
  mcpTools?: string[]; // Allowed MCP tools for this persona
}
...
interface SwayamClient {
  ...
  joinedAt: number;
  toolHistory: Array<{ tool: string; result: unknown; timestamp: number }>;
  // Wake word state
  wakeWordEnabled: boolean;
  wakeWordActive: boolean;  // True when wake word detected, waiting for command
  wakeWordTimeout?: NodeJS.Timeout;
  // Streaming state
  streamingEnabled: boolean;
  // Phase 12.1: Intelligence state
  pendingPlan?: TodoPlan;           // Plan waiting for user confirmation
  lastAnalysis?: ConversationAnalysis;  // Last conversation analysis result
  intelligenceEnabled: boolean;     // Use @ankr/intelligence for processing
}

type SwayamMessage =
  | { type: 'join'; sessionId: string; userId: string; language: string; persona?: string; wakeWord?: boolean; streaming?: boolean }
  | { type: 'audio'; data: string; continuous?: boolean }  // continuous = wake word listening mode
  | { type: 'text'; text: string }
  | { type: 'execute'; code: string; language: string }
  | { type: 'tool'; name: string; args: Record<string, unknown> }
  | { type: 'wake_word_config'; enabled: boolean; keywords?: string[] }
  | { type: 'streaming_config'; enabled: boolean }
  | { type: 'plan_confirm'; planId: string; confirmed: boolean }  // Phase 12.1: Plan confirmation
  | { type: 'plan_execute'; planId: string }                      // Phase 12.1: Execute plan
  | { type: 'plan_cancel'; planId: string }                       // Phase 12.1: Cancel plan
  | { type: 'leave' };
...
const INTENT_TOOL_MAP: Record<string, { tools: string[]; priority: number }> = {
...
export class SwayamWebSocketHandlerMCP {
  ...
  private toolRegistry: ReturnType<typeof getToolRegistry> | null = null;
  private toolMap: Map<string, MCPTool> = new Map();

  // Phase 12.1: Conversational Intelligence
  private conversationAI: ConversationAI;
  private intelligenceEnabled: boolean = false;
  private activePlans: Map<string, TodoPlan> = new Map();

  // Wake Word Detection
  private wakeWordEnabled: boolean = false;
  private wakeWordDetector: WakeWordDetector | null = null;
  private wakeWordKeywords: string[] = ['hey swayam', 'swayam'];
  private wakeWordSensitivity: number = 0.7;
  private wakeWordTimeoutMs: number = 10000; // 10 seconds to speak after wake word
   ...
   * Initialize Conversational Intelligence (Phase 12.1)
   */
  private async initIntelligence(config?: { verdaccioUrl?: string; autoExecute?: boolean }): Promise<void> {
    try {
      // Initialize and index packages
      await this.conversationAI.initialize();

      // Register progress callback for WebSocket updates
      this.conversationAI.onProgress((event: ProgressEvent) => {
   ...
   */
  private initMCP(config?: { logisticsRagUrl?: string; databaseUrl?: string }): void {
    try {
      ...
      setupAllTools({ databaseUrl: config?.databaseUrl });
   ...
   * Initialize Wake Word Detection
   */
  private initWakeWord(config?: { keywords?: string[]; sensitivity?: number }): void {
    try {
      this.wakeWordKeywords = config?.keywords || ['hey swayam', 'swayam', 'हे स्वयं'];
      this.wakeWordSensitivity = config?.sensitivity || 0.7;

      this.wakeWordDetector = createWakeWordDetector(this.wakeWordKeywords, {
   ...
   * Initialize Streaming TTS
   */
  private initStreamingTTS(config?: { chunkSize?: number; maxConcurrent?: number }): void {
    try {
      this.streamingTTS = createStreamingTTS({
  ...
  private async handleMessage(ws: WebSocket, message: SwayamMessage): Promise<void> {
    ...
    switch (message.type) {
      case 'join':
        ...
        break;
      case 'audio':
        await this.handleAudio(ws, message.data, (message as any).continuous);
        break;
      case 'text':
        // Update language if sent with message
        if (message.language) {
          const client = this.clients.get(ws);
          if (client && client.language !== message.language) {
            client.language = message.language as SupportedLanguage;
            console.log(`🌐 Language updated from text message: ${message.language}`);
          }
        }
      ...
      case 'leave':
        ...
        break;
      case 'language':
        // Handle language change from client
        this.handleLanguageChange(ws, message.language);
        break;
    }
  }
   ...
   * Handle language change from client
   */
  private handleLanguageChange(ws: WebSocket, newLanguage: string): void {
    const client = this.clients.get(ws);
    if (!client) return;

    const oldLanguage = client.language;
    client.language = newLanguage as SupportedLanguage;

    console.log(`🌐 Language changed for ${client.userId}: ${oldLanguage} → ${newLanguage}`);
   ...
   */
  private handleWakeWordConfig(ws: WebSocket, msg: { enabled: boolean; keywords?: string[] }): void {
    const client = this.clients.get(ws);
    if (!client) return;
   ...
   */
  private handleStreamingConfig(ws: WebSocket, msg: { enabled: boolean }): void {
    const client = this.clients.get(ws);
    if (!client) return;
   ...
   */
  private async handlePlanConfirm(ws: WebSocket, planId: string, confirmed: boolean): Promise<void> {
    const client = this.clients.get(ws);
    if (!client) {
   ...
   * Execute a plan
   */
  private async handlePlanExecute(ws: WebSocket, planId: string): Promise<void> {
    const client = this.clients.get(ws);
    if (!client) {
      this.sendError(ws, 'NOT_JOINED', 'Join a session first');
      return;
    ...
    try {
      ...
      const executedPlan = await this.conversationAI.executePlan(
        plan,
   ...
   * Cancel a plan
   */
  private handlePlanCancel(ws: WebSocket, planId: string): void {
    const client = this.clients.get(ws);
    if (!client) return;
  ...
  private async handleJoin(
    ws: WebSocket,
    msg: { sessionId: string; userId: string; language: string; persona?: string; wakeWord?: boolean; streaming?: boolean; intelligence?: boolean }
  ): Promise<void> {
  ...
  private async handleAudio(ws: WebSocket, audioBase64: string, continuous: boolean = false): Promise<void> {
    const client = this.clients.get(ws);
    if (!client) {
      this.sendError(ws, 'NOT_JOINED', 'Join a session first');
      return;
    ...
    try {
      ...
      if (continuous && client.wakeWordEnabled && this.wakeWordDetector) {
        ...
        if (wakeWordResult.detected) {
          ...
          if (client.wakeWordTimeout) clearTimeout(client.wakeWordTimeout);
  ...
  private async processAndRespond(
    ...
    try {
      ...
      if (client.intelligenceEnabled && this.intelligenceEnabled) {
        ...
        const analysis = await this.conversationAI.analyze(
          userText,
          client.sessionId,
          { persona: client.persona, language: client.language }
        );
      ...
      const intent = client.lastAnalysis
        ? {
            ...
            entities: client.lastAnalysis.entities as Record<string, string>,
          }
      ...
      if (client.streamingEnabled && this.streamingTTS && voiceText.length > 100) {
        ...
        // Stream audio chunks
        try {
          await this.streamingTTS.streamWithCallback(
            voiceText.substring(0, 1000), // Limit for streaming
            client.language,
            (chunk: AudioChunk) => {
              this.send(ws, {
                type: 'audio_chunk',
      ...
      updatePattern(
        `intent:${intent.type}:${client.persona}`,
        `${intent.type} intent in ${client.persona}`,
        { success: true, toolsUsed: toolResults.length },
   ...
   */
  private classifyIntent(text: string, persona: string): IntentResult {
    ...
    if (
      /(?:truck|route|delivery|shipment|ट्रक|रूट|डिलीवरी|load|भार|माल)/i.test(lower) ||
  ...
  private async executeIntentTools(
    ...
    for (const toolName of intent.suggestedTools) {
      ...
      try {
        ...
        updatePattern(
          `tool:${toolName}:${client.persona}`,
          `Successful ${toolName} usage in ${client.persona}`,
          { success: true, intent: intent.type },
          [{ query: userText.substring(0, 100) }]
        ).catch(() => {});

        // Stop after first successful search to avoid redundancy
        if (toolName.includes('search') && result) break;
      } catch (error) {
  ...
  private async executeTool(toolName: string, args: Record<string, unknown>): Promise<unknown> {
    ...
    const result: MCPResult = await tool.execute(args);
   ...
   */
  private buildToolContext(toolResults: Array<{ tool: string; result: unknown }>): string {
    ...
    for (const { tool, result } of toolResults) {
      ...
      if (tool === 'logistics_search' && Array.isArray(result)) {
        ...
        parts.push(
          `Search Results (${docs.length} documents):\n` +
            docs
              .map(
                (d: any, i: number) =>
                  `${i + 1}. ${d.title || 'Document'}\n   ${(d.content || '').substring(0, 200)}...`
              )
              .join('\n\n')
        );
        ...
        parts.push(
          `Compliance Check:\n` +
            ...
            `Rules: ${(compliance.rules || []).map((r: any) => `- ${r.rule}: ${r.status}`).join('\n')}\n` +
  ...
  getToolsByCategory(category: string): string[] {
    ...
    const categories = this.toolRegistry.getCategories() as unknown as Record<string, MCPTool[]>;
    return (categories[category] || []).map((t: MCPTool) => t.name);
  }
   ...
   */
  async executeToolDirect(toolName: string, params: Record<string, unknown>): Promise<unknown> {
    const tool = this.toolMap.get(toolName);
