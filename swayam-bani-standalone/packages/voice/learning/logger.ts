 ...
 */
export async function logLearningEvent(event: LearningEventInput): Promise<string | null> {
  try {
 ...
 * Log tool usage
 */
export async function logToolUsage(usage: ToolUsageInput): Promise<number | null> {
  try {
    const result = await pool.query(
...
export async function updatePattern(
  ...
  try {
    await pool.query(
      `INSERT INTO learning_patterns (name, description, outcome, conditions, occurrences, confidence, last_seen)
       ...
       ON CONFLICT (name) DO UPDATE SET
         ...
         END`,
      [name, description, JSON.stringify(outcome), JSON.stringify(conditions)]
    );
  } catch (error) {
 ...
 */
export async function getAnalytics(startDate?: Date, endDate?: Date): Promise<any> {
  try {
 ...
 */
export async function getTopPatterns(limit: number = 10): Promise<any[]> {
  try {
    const result = await pool.query(
...
export async function runDailyAggregation(): Promise<void> {
  try {
    ...
    const patternStats = await pool.query(
      `SELECT COUNT(*) as patterns_discovered, AVG(confidence) as avg_confidence
       FROM learning_patterns
       WHERE last_seen >= $1`,
      [today]
    );
    ...
    const personaStats = await pool.query(
      `SELECT json_agg(json_build_object('persona', persona, 'count', cnt) ORDER BY cnt DESC) as top_personas
       FROM (
